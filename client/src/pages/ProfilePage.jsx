        ...
        <form onSubmit={handleSubmit} className='p-10 flex flex-col gap-5 flex-1'>
          ...
          <label htmlFor="avatar" className='flex items-center gap-4 cursor-pointer'>
            ...
            <img src={selectedImage ? URL.createObjectURL(selectedImage) : assets.avatar_icon}
              alt="Avatar" className={`w-12 h-12 ${selectedImage && 'rounded-full'}`} />
          ...
          <button type="submit" className="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500">
            Save
          </button>

        </form>
        <img className = {`max-w-44 aspect-square rounded-full mx-10 max-sm:mt-10 $
          {selectedImage && 'rounded-full'}`} src = {assets.logo_icon} alt="Logo" />
      </div>
    </div>
